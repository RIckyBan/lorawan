...
#include "ns3/custom-periodic-sender-helper.h"
...
NS_LOG_COMPONENT_DEFINE ("CustomPeriodicSenderHelper");

CustomPeriodicSenderHelper::CustomPeriodicSenderHelper ()
{
  m_factory.SetTypeId ("ns3::CustomPeriodicSender");
...
void
CustomPeriodicSenderHelper::SetAttribute (std::string name, const AttributeValue &value)
{
  m_factory.Set (name, value);
}

ApplicationContainer
CustomPeriodicSenderHelper::Install (Ptr<Node> node) const
{
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer
CustomPeriodicSenderHelper::Install (NodeContainer c) const
{
...
Ptr<Application>
CustomPeriodicSenderHelper::InstallPriv (Ptr<Node> node) const
{
  NS_LOG_FUNCTION (this << node);

  Ptr<CustomPeriodicSender> app = m_factory.Create<CustomPeriodicSender> ();
...
void
CustomPeriodicSenderHelper::SetPeriod (Time period)
{
  m_period = period;
}

void
CustomPeriodicSenderHelper::SetPacketSizeRandomVariable (Ptr <RandomVariableStream> rv)
{
  m_pktSizeRV = rv;
}

void
CustomPeriodicSenderHelper::SetPacketSize (uint8_t size)
{
  m_pktSize = size;
}
